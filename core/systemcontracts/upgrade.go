...
func UpgradeBuildInSystemContract(config *params.ChainConfig, blockNumber *big.Int, statedb *state.StateDB) error {
	...
	if config.IsPrimordialPanoptisBlock(blockNumber.Uint64()) {
		configs, err := primordialPanoptisUpgrade(config)
		if err != nil {
			return err
		}
		applySystemContractUpgrade(&Upgrade{
			UpgradeName: "PrimordialPanoptis",
			Configs:     configs,
...
func primordialPanoptisUpgrade(config *params.ChainConfig) ([]*UpgradeConfig, error) {
	if config.Parlia.SystemContracts == nil {
		return nil, errors.New("Missing systemContracts in parlia config for PrimordialPanoptis fork")
	}
