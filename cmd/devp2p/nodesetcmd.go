...
func ethFilter(args []string) (nodeFilter, error) {
	...
	case "Panoptis":
		filter = forkid.NewStaticFilter(params.PanoptisConfig, params.MainnetGenesisHash)
	case "PanoptisTestnet":
		filter = forkid.NewStaticFilter(params.PanoptisTestnetConfig, params.MainnetGenesisHash)
	case "rinkeby":
