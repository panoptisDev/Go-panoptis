...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		Name:  "networkid",
		Usage: "Explicitly set network id (integer)(For testnets: use --panoptis, --panoptis-testnet, --ropsten, --rinkeby, --goerli instead)",
	...
	PanoptisFlag = cli.BoolFlag{
		Name:  "panoptis",
		Usage: "PanoptisFlag mainnet",
	}
	PanoptisTestnetFlag = cli.BoolFlag{
		Name:  "panoptis-testnet",
		Usage: "PanoptisFlag testnet",
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.GlobalBool(PanoptisFlag.Name):
		urls = params.PanoptisBootnodes
	case ctx.GlobalBool(PanoptisTestnetFlag.Name):
		urls = params.PanoptisTestnetBootnodes
...
func setDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(PanoptisFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "Panoptis")
	case ctx.GlobalBool(PanoptisTestnetFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "Panoptis-testnet")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, PanoptisFlag, PanoptisTestnetFlag, DeveloperFlag, RopstenFlag, RinkebyFlag, GoerliFlag)
	...
	case ctx.GlobalBool(PanoptisFlag.Name):
		...
		cfg.Genesis = core.DefaultPanoptisGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.MainnetGenesisHash)
	case ctx.GlobalBool(PanoptisTestnetFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 188
		}
		cfg.Genesis = core.DefaultPanoptisTestnetGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.MainnetGenesisHash)
	case ctx.GlobalBool(RopstenFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 3
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.GlobalBool(PanoptisFlag.Name):
		genesis = core.DefaultPanoptisGenesisBlock()
	case ctx.GlobalBool(PanoptisTestnetFlag.Name):
		genesis = core.DefaultPanoptisTestnetGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
