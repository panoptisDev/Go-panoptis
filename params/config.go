...
var (
	...
	MainnetChainConfig = &ChainConfig{
		...
		//PrimordialPanoptisBlock: big.NewInt(12_965_000),
	...
	PanoptisConfig = &ChainConfig{
		ChainID:              big.NewInt(´187),
		HomesteadBlock:       big.NewInt(1_150_000),
		DAOForkBlock:         big.NewInt(1_920_000),
		DAOForkSupport:       true,
		EIP150Block:          big.NewInt(2_463_000),
		...
		BerlinBlock:          big.NewInt(12_244_000),
		LondonBlock:          big.NewInt(12_965_000),
		PrimordialPanoptisBlock: big.NewInt(13_224_746),
		Parlia: &ParliaConfig{
			Period: 3,
			Epoch:  200,
			Era:    144,
	...
	PanoptisTestnetConfig = &ChainConfig{
		...
		Parlia: &ParliaConfig{
			...
			Epoch:           200,
			Era:             144,
			InitValidators:  &panoptisChainTestnetInitValidators,
			SystemContracts: &panoptisChainTestnetSystemContracts,
			Treasury:        &panoptisChainTestnetTreasury,
		},
	}
...
type ChainConfig struct {
	...
	PrimordialPanoptisBlock *big.Int `json:"primordialPanoptisBlock,omitempty" toml:",omitempty"` // primordialPanoptisBlock switch block (nil = no fork, 0 = already activated)
...
type ParliaConfig struct {
	...
	InitValidators  *[]string         `json:"initValidators,omitempty" toml:",omitempty"`  // The list of consensus addresses for the initial validatorSet, used for the PrimordialPanoptisBlock only
	SystemContracts *[]SystemContract `json:"systemContracts,omitempty" toml:",omitempty"` // The list of system contracts to deploy during, used for the PrimordialPanoptisBlock only
	Treasury        *Treasury         `json:"treasury,omitempty" toml:",omitempty"`        // An optional treasury which will receive allocations durign the PrimordialPanoptisBlock only
}
...
func (c *ChainConfig) String() string {
	...
	return fmt.Sprintf("{ChainID: %v Homestead: %v DAO: %v DAOSupport: %v EIP150: %v EIP155: %v EIP158: %v Byzantium: %v Constantinople: %v Petersburg: %v Istanbul: %v, Muir Glacier: %v, Ramanujan: %v, Niels: %v, MirrorSync: %v, Berlin: %v, PrimordialPanoptis: %v, London: %v, Engine: %v}",
		c.ChainID,
		c.HomesteadBlock,
		c.DAOForkBlock,
		c.DAOForkSupport,
...
// IsPrimordialPanoptis returns whether num is either equal to the PrimordialPanoptis fork block or greater.
func (c *ChainConfig) IsPrimordialPanoptis(num *big.Int) bool {
	return isForked(c.PrimordialPanoptisBlock, num)
}

// IsPrimordialPanoptisBlock returns whether or not the given block is the primordial panoptis block.
func (c *ChainConfig) IsPrimordialPanoptisBlock(number uint64) bool {
	// Returns whether or not the given block is the PrimordialPanoptisBlock.
	return c.PrimordialPanoptisBlock != nil && number == c.PrimordialPanoptisBlock.Uint64()
}

// Returns true if there is a PrimordialPanoptis block in the future, indicating this chain
// should still be evaluated using the ethash consensus engine and with mainnet ChainID.
func (c *ChainConfig) PrimordialPanoptisAhead(number *big.Int) bool {
	return c.PrimordialPanoptisBlock != nil && c.PrimordialPanoptisBlock.Cmp(number) > 0
}
...
func (c *ChainConfig) CheckConfigForkOrder() error {
	...
	for _, cur := range []fork{
		...
		{name: "primordialPanoptisBlock", block: c.PrimordialPanoptisBlock},
	} {
		if lastFork.name != "" {
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, head *big.Int) *ConfigCompatError {
	...
	// allow mismatching ChainID if we're at the PrimordialPanoptisBlock
	if c.IsEIP158(head) && !configNumEqual(c.ChainID, newcfg.ChainID) && !newcfg.IsPrimordialPanoptisBlock(head.Uint64()+1) {
	...
	if isForkIncompatible(c.PrimordialPanoptisBlock, newcfg.PrimordialPanoptisBlock, head) {
		return newCompatError("primordialPanoptis fork block", c.PrimordialPanoptisBlock, newcfg.PrimordialPanoptisBlock)
	}
