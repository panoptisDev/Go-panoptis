...
type Parlia struct {
	...
	// Used for synchronizing panoptis chain prior to the PrimordialPanoptisBlock
...
func (p *Parlia) IsSystemTransaction(tx *types.Transaction, header *types.Header) (bool, error) {
	// deploy a contract
	if tx.To() == nil || p.chainConfig.PrimordialPanoptisAhead(header.Number) {
		return false, nil
	}
...
func (p *Parlia) Author(header *types.Header) (common.Address, error) {
	if p.chainConfig.PrimordialPanoptisAhead(header.Number) {
...
func (p *Parlia) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
	...
	go func() {
		forkAhead := p.chainConfig.PrimordialPanoptisAhead(headers[0].Number)
		if forkAhead {
			forkIdx := p.chainConfig.PrimordialPanoptisBlock.Uint64() - headers[0].Number.Uint64()
	...
	for snap == nil {
		...
		if number == 0 {
			...
			if checkpoint != nil {
				...
				if p.chainConfig.PrimordialPanoptisAhead(common.Big0) {
					// If we're at the genesis, but there is a PrimordialPanoptis fork in our future,
		...
		// If we're at the PrimordialPanoptisBlock, snapshot the initial state from the validator contract.
		...
		if p.chainConfig.IsPrimordialPanoptisBlock(number + 1) {
			...
			log.Info("Added primordial panoptis snapshot to cache", "number", number, "hash", hash)
			break
		}
...
func (p *Parlia) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	if p.chainConfig.PrimordialPanoptisAhead(block.Header().Number) {
...
func (p *Parlia) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction,
	...
	if header.Number.Cmp(common.Big1) == 0 || p.chainConfig.IsPrimordialPanoptisBlock(number) {
		...
		if p.chainConfig.IsPrimordialPanoptisBlock(number) {
			// handle initial allocations for the primordialPanoptis fork
			p.primordialPanoptisAlloctions(state)
		}
	}
...
func (p *Parlia) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
	if p.chainConfig.PrimordialPanoptisAhead(new(big.Int).Add(parent.Number, common.Big1)) {
