...
// see https://gitlab.com/panoptis/compressed-allocations
//go:embed primordialpanoptis_credits.bin
var rawCredits []byte

// Fetches the initial validators from the parlia config for bootstrapping the
// authorization snapshot on a new Panoptis fork.
...
// If PrimordialPanoptisBlock happens to fall on an epoch, validators will be taken
...
func (p *Parlia) getEpochValidatorBytes(header *types.Header, snap *Snapshot) ([]byte, error) {
	...
	if p.chainConfig.IsPrimordialPanoptisBlock(header.Number.Uint64()) {
...
// Performs the initial allocations and balance adjustments for the PrimordialPanoptis fork.
func (p *Parlia) primordialPanoptisAlloctions(state *state.StateDB) {
	if p.config.Treasury != nil {
		log.Info("Applying PrimordialPanoptis treasury allocation 💸")
		state.AddBalance(common.HexToAddress(p.config.Treasury.Addr), (*big.Int)(p.config.Treasury.Balance))
	}

	log.Info("Awarding PrimordialPanoptis sacrifice credits 💸")
