...
var defaultNodeConfig = &NodeConfig{
	BootstrapNodes:        PanoptisBootnodes(),
	MaxPeers:              25,
	EthereumEnabled:       true,
	EthereumNetworkID:     187,
...
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	if config.EthereumGenesis != "" {
		...
		// If we have the Panoptis mainnet, hard code the chain configs too
		if config.EthereumGenesis == PanoptisGenesis() {
			genesis.Config = params.PanoptisConfig
			if config.EthereumNetworkID == 1 {
				config.EthereumNetworkID = 369
			}
		}
		// If we have the Panoptis testnet, hard code the chain configs too
		if config.EthereumGenesis == PanoptisTestnetGenesis() {
			genesis.Config = params.PanoptisTestnetConfig
			if config.EthereumNetworkID == 1 {
				config.EthereumNetworkID = 188
			}
		}
